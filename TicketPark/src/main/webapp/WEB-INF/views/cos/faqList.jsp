...
<%@ include file="/WEB-INF/layout/cosHeader.jsp" %>
    ...
    <script type="text/javascript">
        ...
        function fn_openFaqWriteForm(){
            var comSubmit = new ComSubmit();
            comSubmit.setUrl("<c:url value='/cos/faqWriteForm' />");
            comSubmit.submit();
        }
         
        function fn_openFaqDetail(obj){
            var comSubmit = new ComSubmit();
            comSubmit.setUrl("<c:url value='/cos/faqDetail' />");
         ...
         function fn_selectFaqList(pageNo){
			...
			comAjax.setUrl("<c:url value='/cos/selectFaqList'/>");
        ...
        function fn_selectFaqListCallback(data){
			...
			if(total == 0){
				...
				$.each(data.list, function(key, value){
					str +=  "<tr>" +
							"<td>" + value.FAQ_NUM + "</td>" +
							"<td class = 'title'>" + "<a href='#this' name='title'>" + value.FAQ_TITLE + "</a>" +
							"<input type='hidden' id='FAQ_NUM' value=" + value.FAQ_NUM + ">" + "</td>" +
							"</tr>";
				});
				body.append(str);
